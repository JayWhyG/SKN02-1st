...
if page == "한국 자동차 보유 현황":
    ...
    for i in year_list:
        ...
        fig_rv.add_trace(go.Bar(name='쏘카', x=categories, y=sorv, text='쏘카', textposition='auto', insidetextanchor='middle'))
        fig_rv.update_layout(barmode='stack', title="매출", xaxis_title="최근 5년", yaxis_title="매출")

        fig_pf = go.Figure()
        fig_pf.add_trace(go.Bar(name='SK', x=categories, y=skpf, text='SK', textposition='auto', insidetextanchor='middle'))
        fig_pf.add_trace(go.Bar(name='롯데', x=categories, y=lopf, text='롯데', textposition='auto', insidetextanchor='middle'))
        fig_pf.add_trace(go.Bar(name='쏘카', x=categories, y=sopf, text='쏘카', textposition='auto', insidetextanchor='middle'))
        fig_pf.update_layout(barmode='stack', title="영업이익", xaxis_title="최근 5년(쏘카는 2022년 제외하고 매년 손실액)", yaxis_title="영업이익")

        fig_rt = go.Figure()
        fig_rt.add_trace(go.Bar(name='SK', x=categories, y=skrt, text='SK', textposition='auto', insidetextanchor='middle'))
        fig_rt.add_trace(go.Bar(name='롯데', x=categories, y=lort, text='롯데', textposition='auto', insidetextanchor='middle'))
